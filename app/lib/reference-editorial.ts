...
export const EDITORIAL: Record<string, Editorial> = {
  ...
  langchain: {
    claim:
      "Let a LangChain agent use optional Jev tool selection for a bounded action, then cross an MCP boundary while the receiver verifies who authorized the exact action.",
    prerequisites: "Python 3.12. No model API key or paid service.",
    run: "./scripts/langchain-reference-check.sh",
    evidence: "28 tests pass, including deterministic Jev-selection cases, zero skipped, and the gate fails on any skip. The reference does not claim arbitrary coding-agent execution.",
    endorsement:
